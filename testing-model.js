const { Record, List, Map } = require('immutable')

const TEMP_CARD_JSON = {
  "_embedded": {
    "cards": [
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Arbor Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "A25",
            "M13",
            "WWK"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "{T}: Untap target Forest.",
          "toughness": "1",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "7168d874-7864-53cb-9a51-3bf6bf5f19b7"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "R"
          ],
          "colorIndicator": [],
          "colors": [
            "G",
            "R"
          ],
          "convertedManaCost": 4,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{2}{R}{G}",
          "multiverseId": "",
          "name": "Bloodbraid Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "3",
          "printings": [
            "ARB",
            "C16",
            "EMA",
            "F10",
            "PC2",
            "PCA",
            "PRM"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Berserker"
          ],
          "supertypes": [],
          "text": "Haste\nCascade (When you cast this spell, exile cards from the top of your library until you exile a nonland card that costs less. You may cast it without paying its mana cost. Put the exiled cards on the bottom of your library in a random order.)",
          "toughness": "2",
          "type": "Creature — Elf Berserker",
          "types": [
            "Creature"
          ],
          "uuid": "d99460f8-dc95-59f6-b901-7e0e7316fc29"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Cylian Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "2",
          "printings": [
            "ALA"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Scout"
          ],
          "supertypes": [],
          "text": "",
          "toughness": "2",
          "type": "Creature — Elf Scout",
          "types": [
            "Creature"
          ],
          "uuid": "af867e36-1bc7-5b68-a8fb-52d3c3fcf784"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Defiant Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "LGN"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "Trample",
          "toughness": "1",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "452aa6c2-4551-5d19-8778-291fe3e3cbbb"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [],
          "convertedManaCost": 3,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{3}",
          "multiverseId": "",
          "name": "Elf Replica",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "2",
          "printings": [
            "MRD"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "{1}{G}, Sacrifice Elf Replica: Destroy target enchantment.",
          "toughness": "2",
          "type": "Artifact Creature — Elf",
          "types": [
            "Artifact",
            "Creature"
          ],
          "uuid": "cc22401f-1e2e-54bc-918f-6f0cd8f8b3da"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Elfhame Druid",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "0",
          "printings": [
            "DOM"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "{T}: Add {G}.\n{T}: Add {G}{G}. Spend this mana only to cast kicked spells.",
          "toughness": "2",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "6c2cab08-b939-5c43-bb71-98226dcae03b"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "W"
          ],
          "colorIndicator": [],
          "colors": [],
          "convertedManaCost": 0,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "",
          "multiverseId": "",
          "name": "Elfhame Palace",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "",
          "printings": [
            "8ED",
            "C17",
            "DDE",
            "INV",
            "TD0",
            "WC03"
          ],
          "rarity": "",
          "subtypes": [],
          "supertypes": [],
          "text": "Elfhame Palace enters the battlefield tapped.\n{T}: Add {G} or {W}.",
          "toughness": "",
          "type": "Land",
          "types": [
            "Land"
          ],
          "uuid": "747a31a4-73e2-5ab5-9caf-e5e65143db01"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Elfhame Sanctuary",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "",
          "printings": [
            "INV"
          ],
          "rarity": "",
          "subtypes": [],
          "supertypes": [],
          "text": "At the beginning of your upkeep, you may search your library for a basic land card, reveal that card, and put it into your hand. If you do, you skip your draw step this turn and shuffle your library.",
          "toughness": "",
          "type": "Enchantment",
          "types": [
            "Enchantment"
          ],
          "uuid": "d7659df0-22ca-56b8-a8aa-2e369f69af78"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 3,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{2}{G}",
          "multiverseId": "",
          "name": "Farhaven Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "C13",
            "C14",
            "C18",
            "CMA",
            "PS11",
            "SHM"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "When Farhaven Elf enters the battlefield, you may search your library for a basic land card, put it onto the battlefield tapped, then shuffle your library.",
          "toughness": "1",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "8255e546-b7b4-595b-b20e-4cb6778ca3b1"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "W"
          ],
          "colorIndicator": [],
          "colors": [
            "G",
            "W"
          ],
          "convertedManaCost": 7,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{3}{G}{G}{W}{W}",
          "multiverseId": "",
          "name": "Gabriel Angelfire",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "4",
          "printings": [
            "CHR",
            "LEG",
            "ME3"
          ],
          "rarity": "",
          "subtypes": [
            "Angel"
          ],
          "supertypes": [
            "Legendary"
          ],
          "text": "At the beginning of your upkeep, choose flying, first strike, trample, or rampage 3. Gabriel Angelfire gains that ability until your next upkeep. (Whenever a creature with rampage 3 becomes blocked, it gets +3/+3 until end of turn for each creature blocking it beyond the first.)",
          "toughness": "4",
          "type": "Legendary Creature — Angel",
          "types": [
            "Creature"
          ],
          "uuid": "8268d7f4-0418-596c-95d0-45037e461642"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Glistener Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "F12",
            "NPH",
            "PRM"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Warrior"
          ],
          "supertypes": [],
          "text": "Infect (This creature deals damage to creatures in the form of -1/-1 counters and to players in the form of poison counters.)",
          "toughness": "1",
          "type": "Creature — Elf Warrior",
          "types": [
            "Creature"
          ],
          "uuid": "212a3cf9-2435-5a43-bb3b-06aac4e510c4"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Nomadic Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "2",
          "printings": [
            "DDE",
            "INV"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Nomad"
          ],
          "supertypes": [],
          "text": "{1}{G}: Add one mana of any color.",
          "toughness": "2",
          "type": "Creature — Elf Nomad",
          "types": [
            "Creature"
          ],
          "uuid": "13a072f8-401e-5424-9dc3-cb6dcdc5e2e1"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Selfie Preservation",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "",
          "printings": [
            "UST"
          ],
          "rarity": "",
          "subtypes": [],
          "supertypes": [],
          "text": "Search your library for a basic land card and reveal it. If there's a tree in its art, put it onto the battlefield tapped. Otherwise, put it into your hand. Then shuffle your library.",
          "toughness": "",
          "type": "Sorcery",
          "types": [
            "Sorcery"
          ],
          "uuid": "bc2e76ef-ea09-5f6e-8b48-cfa2f69610b2"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "U"
          ],
          "colorIndicator": [],
          "colors": [
            "G",
            "U"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{X}{G}{U}",
          "multiverseId": "",
          "name": "Skyrider Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "0",
          "printings": [
            "BFZ"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Warrior",
            "Ally"
          ],
          "supertypes": [],
          "text": "Flying\nConverge — Skyrider Elf enters the battlefield with a +1/+1 counter on it for each color of mana spent to cast it.",
          "toughness": "0",
          "type": "Creature — Elf Warrior Ally",
          "types": [
            "Creature"
          ],
          "uuid": "4b94e40e-9682-5896-b72c-cdb455d2855a"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "R",
            "W"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Skyshroud Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "TMP",
            "TPR"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "{T}: Add {G}.\n{1}: Add {R} or {W}.",
          "toughness": "1",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "48dd426f-a441-5440-9baf-fc7c845165bd"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G",
            "W"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Sunblade Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "M15"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Warrior"
          ],
          "supertypes": [],
          "text": "Sunblade Elf gets +1/+1 as long as you control a Plains.\n{4}{W}: Creatures you control get +1/+1 until end of turn.",
          "toughness": "1",
          "type": "Creature — Elf Warrior",
          "types": [
            "Creature"
          ],
          "uuid": "bfa517b2-47f1-5000-a55c-66037bf8edba"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 4,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{3}{G}",
          "multiverseId": "",
          "name": "Symbiotic Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "2",
          "printings": [
            "ONS",
            "TD2"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "When Symbiotic Elf dies, create two 1/1 green Insect creature tokens.",
          "toughness": "2",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "4bff20c5-9ca4-555c-ab2b-3e0a35c901c2"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Taunting Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "0",
          "printings": [
            "ONS",
            "UDS"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "All creatures able to block Taunting Elf do so.",
          "toughness": "1",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "4efc9255-e4f4-5ae7-b695-55ab4c3ef7d9"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 3,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{2}{G}",
          "multiverseId": "",
          "name": "Timberwatch Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "C14",
            "CMA",
            "DD1",
            "EMA",
            "EVG",
            "LGN",
            "PSAL"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "{T}: Target creature gets +X/+X until end of turn, where X is the number of Elves on the battlefield.",
          "toughness": "2",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "678a567d-9508-51ab-82d4-d8e96acc1725"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "B",
            "G",
            "U"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Urborg Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "APC"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "{T}: Add {B}, {G}, or {U}.",
          "toughness": "1",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "92949fec-dd01-5bfe-8e67-a247b37927b6"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Warren-Scourge Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "LRW"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Warrior"
          ],
          "supertypes": [],
          "text": "Protection from Goblins",
          "toughness": "1",
          "type": "Creature — Elf Warrior",
          "types": [
            "Creature"
          ],
          "uuid": "a52eff6b-59de-5b06-bcb5-90b2f5edf3af"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Weatherseed Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "ULG"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "{T}: Target creature gains forestwalk until end of turn. (It can't be blocked as long as defending player controls a Forest.)",
          "toughness": "1",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "05250084-836a-5728-bf52-334ed7aba1c9"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 1,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{G}",
          "multiverseId": "",
          "name": "Willow Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "S00",
            "S99"
          ],
          "rarity": "",
          "subtypes": [
            "Elf"
          ],
          "supertypes": [],
          "text": "",
          "toughness": "1",
          "type": "Creature — Elf",
          "types": [
            "Creature"
          ],
          "uuid": "b425d5c2-187b-5719-992d-8a1523f6572c"
        }
      ],
      [
        {
          "artist": "",
          "borderColor": "",
          "colorIdentity": [
            "G"
          ],
          "colorIndicator": [],
          "colors": [
            "G"
          ],
          "convertedManaCost": 2,
          "duelDeck": "",
          "faceConvertedManaCost": 0,
          "flavorText": "",
          "frameEffect": "",
          "frameVersion": "",
          "hasFoil": false,
          "hasNonFoil": false,
          "isAlternative": false,
          "isFoilOnly": false,
          "isOnlineOnly": false,
          "isOversized": false,
          "isReserved": false,
          "isTimeshifted": false,
          "layout": "normal",
          "loyalty": "",
          "manaCost": "{1}{G}",
          "multiverseId": "",
          "name": "Wirewood Elf",
          "names": [],
          "number": "",
          "originalText": "",
          "originalType": "",
          "power": "1",
          "printings": [
            "ONS"
          ],
          "rarity": "",
          "subtypes": [
            "Elf",
            "Druid"
          ],
          "supertypes": [],
          "text": "{T}: Add {G}.",
          "toughness": "2",
          "type": "Creature — Elf Druid",
          "types": [
            "Creature"
          ],
          "uuid": "8359e2c3-f084-503f-bb76-4736e55bff93"
        }
      ]
    ]
  },
  "_links": {
    "self": {
      "href": "/api/cards/?name=elf&page=1&color=G"
    }
  }
}

const results = TEMP_CARD_JSON._embedded.cards

const CARD_PROPERTIES =
  [ 'artist'
  , 'borderColor'
  , 'colorIdentity'
  , 'colorIndicator'
  , 'colors'
  , 'convertedManaCost'
  , 'duelDeck'
  , 'faceConvertedManaCost'
  , 'flavorText'
  , 'frameEffect'
  , 'frameVersion'
  , 'hasFoil'
  , 'hasNonFoil'
  , 'isAlternative'
  , 'isFoilOnly'
  , 'isOnlineOnly'
  , 'isOversized'
  , 'isReserved'
  , 'isTimeshifted'
  , 'layout'
  , 'loyalty'
  , 'manaCost'
  , 'multiverseId'
  , 'name'
  , 'names'
  , 'number'
  , 'originalText'
  , 'originalType'
  , 'power'
  , 'printings'
  , 'rarity'
  , 'subtypes'
  , 'supertypes'
  , 'text'
  , 'toughness'
  , 'type'
  , 'types'
  , 'uuid'
  ]

const generateDefaultConfig = () => {
  const config = {}

  CARD_PROPERTIES.forEach(property => {
    config[property] = undefined
  })

  return config
}

const CreateClassRecord = (config) => {
  return Record(config)
}

const Card = class {
  constructor(card) {
    this.DEFAULT_CONFIG = generateDefaultConfig()
    this.CARD_RECORD = CreateClassRecord(this.DEFAULT_CONFIG)
    this.createCardInstance = this.createCardInstance.bind(this)
    this.CARD_INSTANCE = this.createCardInstance(card)
  }

  createCardInstance(card) {
    const INSTANCE_CONFIG = this.DEFAULT_CONFIG

    CARD_PROPERTIES.forEach(property => {
      INSTANCE_CONFIG[property] = card[property]
    })

    const record = new this.CARD_RECORD(INSTANCE_CONFIG)

    return record
  }

  getRecord() {
    return this.CARD_INSTANCE
  }

  // setProperties(property, card) {
  //   CARD_PROPERTIES.forEach(property => {
  //     this[property] = card[property]
  //   }) 

  //   switch(property) {
  //     case 'artist': {
  //       return card[property]
  //     }
  //     case 'borderColor': {
  //       return card[property]
  //     }
  //     case 'colorIdentity': {
  //       return card[property]
  //     }
  //     case 'colorIndicator': {
  //       return card[property]
  //     }
  //     case 'colors': {
  //       return card[property]
  //     }
  //     case 'convertedManaCost': {
  //       return card[property]
  //     }
  //     case 'duelDeck': {
  //       return card[property]
  //     }
  //     case 'faceConvertedManaCost': {
  //       return card[property]
  //     }
  //     case 'flavorText': {
  //       return card[property]
  //     }
  //     case 'frameEffect': {
  //       return card[property]
  //     }
  //     case 'frameVersion': {
  //       return card[property]
  //     }
  //     case 'hasFoil': {
  //       return card[property]
  //     }
  //     case 'hasNonFoil': {
  //       return card[property]
  //     }
  //     case 'isAlternative': {
  //       return card[property]
  //     }
  //     case 'isFoilOnly': {
  //       return card[property]
  //     }
  //     case 'isOnlineOnly': {
  //       return card[property]
  //     }
  //     case 'isOversized': {
  //       return card[property]
  //     }
  //     case 'isReserved': {
  //       return card[property]
  //     }
  //     case 'isTimeshifted': {
  //       return card[property]
  //     }
  //     case 'layout': {
  //       return card[property]
  //     }
  //     case 'loyalty': {
  //       return card[property]
  //     }
  //     case 'manaCost': {
  //       return card[property]
  //     }
  //     case 'multiverseId': {
  //       return card[property]
  //     }
  //     case 'name': {
  //       return card[property]
  //     }
  //     case 'names': {
  //       return card[property]
  //     }
  //     case 'number': {
  //       return card[property]
  //     }
  //     case 'originalText': {
  //       return card[property]
  //     }
  //     case 'originalType': {
        
  //     }
  //     case 'power': {
        
  //     }
  //     case 'printings': {
        
  //     }
  //     case 'rarity': {
        
  //     }
  //     case 'subtypes': {
        
  //     }
  //     case 'supertypes': {
        
  //     }
  //     case 'text': {
        
  //     }
  //     case 'toughness': {
        
  //     }
  //     case 'type': {
        
  //     }
  //     case 'types': {
        
  //     }
  //     case 'uuid': {
        
  //     }

  //   }
  // }
}

let cards = List([])
results.forEach(cardGroup => {
  let group = List([])

  cardGroup.forEach(card => {
    const instance = new Card(card)
    const record = instance.getRecord()
    group = group.push(record)
  })

  cards = cards.push(group)
})
